...
def configure_jwt(app):
    ...
    freshness_in_minutes = 1  # 토큰을 어느정도로 관리 해줄거냐 짧게 셋팅
    app.config["JWT_ACCESS_TOKEN_EXPIRES"] = freshness_in_minutes * 60 # 1 hour
    jwt.init_app(app)

    # claim: 청구하다
    # 추가적인 정보를 토근에 넣고 싶을 때 사용
    @jwt.additional_claims_loader # @데코레이터 
    def add_claims_to_jwt(identity):  # 데이터가 들어왔을때 이 사람이 어떤 유저인지 아닌지 구분
        if identity == 1:
            return {"is_admin": True}
        return {"is_admin": False}
    
    # 토큰이 블록리스트에 있는지 확인하는 함수
    # 블록리스트에 있으면 해당 토큰이 유효하지 않다고 판다
    @jwt.token_in_blocklist_loader
    def check_if_token_in_blocklist(jwt_header, jwt_payload):
        # jti = jwt id
        return jwt_payload["jti"] in BLOCKLIST  # jti 값을 가져오고 블록리스트에 있으면 이 사람은 차단 당한 사람
    
    # 만료된 토큰이 사용되었을 때 실행되는 함수
    @jwt.expired_token_loader  # 시간이 지났을 시 유저한테 알려주는 메세지
    def expired_token_callback(jwt_header, jwt_payload):
        return jsonify({"msg": "Token expired", "error": "token_expired"}), 401
    
    # 유효하지 않은 토큰이 사용되었을 때 실행되는 함수
    # 토큰의 서명이나 수조가 유효하지 않을 때 실행됩니다. 주로 토큰 자체의 문제로 발생하는 경우에 해당합니다.
